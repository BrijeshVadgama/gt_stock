...
export const registerSoftware = async (req, res) => {
  try {
    ...
    try {
      ...
      if (application === true) {
        const uniqueString =
          `${software_code}-${db_server_name}-${data_path}-${db_name}-${db_username}-${result[0].__id}`.toUpperCase();
      ...
      if (store === true) {
        const uniqueString =
          `${software_code}-${store_server_name}-${store_data_path}-${store_db_name}-${store_db_username}-${result[0].__id}`.toUpperCase();
        const uniqueCode = crypto
          .createHash("sha256")
          .update(uniqueString)
          .digest("hex")
...
export const verifyOTP = async (req, res) => {
  ...
  const token = jwt.sign({ phone_no }, JWT_SECRET, { expiresIn: "1y" });
      ...
      for (let user of results) {
        if (
          ...
          const token = jwt.sign(
            { company_code, mobile_no, password },
            JWT_SECRET,
            { expiresIn: "1y" }
          );
          if (!sequelize) {
